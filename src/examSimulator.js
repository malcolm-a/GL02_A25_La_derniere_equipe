...
function shuffle(array) {
  // algorithme de Fisher-Yales, permet de mélanger un array aléatoirement
  ...
  for (const q of exam.questions) {
    if (q.type === "multiple_choice") {
      ...
      for (let i = 0; i < q.pairs.length; i++) {
        // récupérer les left & right pr former des paires random
...
export async function summaryExam(exam, list_answers) {
  if (list_answers.length != exam.questions.length) {
    console.log(
      "Erreur lors de la simulation de réponses.  " +
        list_answers.length +
        "   " +
    ...
    for (let i = 0; i < list_answers.length; i++) {
      if (exam.questions[i].type === "multiple_choice") {
        ...
        console.log(
          chalk.yellow("Réponse sélectionnée: " + list_answers[i].text + "\n"),
        );
              ...
              console.log(
                chalk.red(
                  "Incorrect: bonne réponse = " +
                    exam.questions[i].choices[c].text +
                    "\n",
                ),
              );
            }
        ...
        if (exam.questions[i].margin) {
          if (
            list_answers[i] >=
              exam.questions[i].answer - exam.questions[i].margin &&
            list_answers[i] <=
              exam.questions[i].answer + exam.questions[i].margin
          ) {
            score++;
        ...
        for (let j = 0; j < list_answers[i].length; j++) {
          for (let k = 0; k < exam.questions[i].pairs.length; k++) {
            if (list_answers[i][j] === exam.questions[i].pairs[k]) {
              console.log(
                chalk.green(
                  list_answers[i][j].left +
                    " -> " +
                    list_answers[i][j].right +
                    "\n",
                ),
              );
            } else {
    ...
    console.log(
      "Score obtenu: " +
        ...
        " / " +
        exam.questions.length +
        " (les short answers et essays valent pour 0.5 et les desc pour 1)\n",
    );
  }
}
